...
/**
 * @func createFileWriteStream
 * @description Create an async write stream.
 *
 * @param {object} params         An object used as interface to define the required params.
 * @param {string} params.desFile The destination file with the written data.
 * @param {string} params.data    The data for being written.
 *
 * @return {Promise<object>} A write stream promise with its respective data.
 */
const createFileWriteStream = async ({ desFile, data }) =>
  new Promise((resolve, reject) => {
    ...
    stream.on("error", (error) => {
      const errMsg =
        "There was an error creating a write stream for the file" +
        ` ${desFile}. ${error}`;
      console.warn(errMsg);
      reject(error);
    });
...
app.post("/houses", async (req, res, next) => {
  try {
    ...
    if (!writeStream) {
      const errMsg =
        "Oops! Something went wrong. The file write stream failed.";
      console.warn(errMsg);
      next(errMsg);
    }
